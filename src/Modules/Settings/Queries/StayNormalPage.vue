            ...
            <section class="py-6 px-4 mb-10 shadow-hoster rounded-b-[10px] rounded-rt bg-white">
                ...
                <div class="mt-4">
                    ...
                    <div class="mt-4">
                        ...
                        <AutoTextArea 
                            v-if="form.in_stay_bad_response_title"
                            :key="forceUpdate"
                            @empty="event => handleEmpty(event,'in_stay_bad_response_title')"
                            :id="'AutoTextArea1'"
                            v-model="form.in_stay_bad_response_title['es']" 
                            :wordLimit="60"
                            mandatory
                        />
                    </div>
                    <div class="mt-4">
                        ...
                        <AutoTextArea 
                            v-if="form.in_stay_bad_response_msg"
                            :key="forceUpdate"
                            @empty="event => handleEmpty(event,'in_stay_bad_response_msg')"
                            :id="'AutoTextArea2'"
                            v-model="form.in_stay_bad_response_msg['es']" 
                            :wordLimit="300"
                            mandatory
                        />
                    </div>
...
onMounted(async() => {
    queriesTexts.value = await querySettingsStore.$getStayBadSettings();
...
function defineMockup(){
    mockupStore.$setIframeUrl('','period=in-stay&openFakeMsg=true&qualification=NORMAL')
    mockupStore.$setInfo1('Edita y guardar para ver tus cambios en tiempo real', '/assets/icons/1.TH.EDIT.OUTLINED.svg')
}

const form = reactive({
    in_stay_bad_response_title:null,
    in_stay_bad_response_msg:null,
})

const submit = async () =>{
    let res  = await querySettingsStore.$updateStayBadSettings(form);
    if(res){
...
function assignValuesToForm(){
    if (queriesTexts.value) {
        form.in_stay_bad_response_title = queriesTexts.value.in_stay_bad_response_title;
        form.in_stay_bad_response_msg = queriesTexts.value.in_stay_bad_response_msg;
    }
}
...
function cancelChanges(){
    ...
    form.in_stay_bad_response_title = { ...oldValues.in_stay_bad_response_title };
    form.in_stay_bad_response_msg = { ...oldValues.in_stay_bad_response_msg };
