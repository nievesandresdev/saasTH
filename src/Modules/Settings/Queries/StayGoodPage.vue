...
onMounted(async() => {
    queriesTexts.value = await querySettingsStore.$getStayGoodSettings();
    assignValuesToForm();
...
function defineMockup(requestActivate){
    ...
    mockupStore.$setIframeUrl('','period=in-stay&openFakeMsg=true&qualification=GOOD&'+activeUrlQuery)
    mockupStore.$setInfo1('Edita y guardar para ver tus cambios en tiempo real', '/assets/icons/1.TH.EDIT.OUTLINED.svg')
}

const form = reactive({
    in_stay_good_request_activate:'true',
    in_stay_good_response_title:null,
    in_stay_good_response_msg:null,
    in_stay_good_request_otas:null,
    in_stay_good_no_request_comment_activate:null,
    in_stay_good_no_request_comment_msg:null,
    in_stay_good_no_request_thanks_title:null,
    in_stay_good_no_request_thanks_msg:null,
})
...
const submit = async () =>{
    updateGeneralForm();
    let res  = await querySettingsStore.$updateStayGoodSettings(form);
    if(res){
...
function assignValuesToForm(){
    if (queriesTexts.value) {
        valuesForm.response_title = queriesTexts.value.in_stay_good_response_title;
        valuesForm.response_msg = queriesTexts.value.in_stay_good_response_msg;
        valuesForm.request_otas = queriesTexts.value.in_stay_good_request_otas;
        valuesForm.no_request_comment_activate = queriesTexts.value.in_stay_good_no_request_comment_activate;
        valuesForm.no_request_comment_msg = queriesTexts.value.in_stay_good_no_request_comment_msg;
        valuesForm.no_request_thanks_title = queriesTexts.value.in_stay_good_no_request_thanks_title;
        valuesForm.no_request_thanks_msg = queriesTexts.value.in_stay_good_no_request_thanks_msg;
        valuesForm.request_activate = String(queriesTexts.value.in_stay_good_request_activate);
    }
}

function updateGeneralForm(){
    form.in_stay_good_response_title = valuesForm.response_title;
    form.in_stay_good_response_msg = valuesForm.response_msg;
    form.in_stay_good_request_otas = valuesForm.request_otas;
    form.in_stay_good_no_request_comment_activate = valuesForm.no_request_comment_activate;
    form.in_stay_good_no_request_comment_msg = valuesForm.no_request_comment_msg;
    form.in_stay_good_no_request_thanks_title = valuesForm.no_request_thanks_title;
    form.in_stay_good_no_request_thanks_msg = valuesForm.no_request_thanks_msg;
    form.in_stay_good_request_activate = valuesForm.request_activate == 'true' ? true : false;
    
}

function cancelChanges(){
    const oldValues = JSON.parse(copyTexts.value);
    valuesForm.response_title = oldValues.in_stay_good_response_title;
    valuesForm.response_msg = { ...oldValues.in_stay_good_response_msg };
    valuesForm.request_otas = oldValues.in_stay_good_request_otas;
    valuesForm.no_request_comment_activate = oldValues.in_stay_good_no_request_comment_activate;
    valuesForm.no_request_comment_msg = { ...oldValues.in_stay_good_no_request_comment_msg };
    valuesForm.no_request_thanks_title = { ...oldValues.in_stay_good_no_request_thanks_title };
    valuesForm.no_request_thanks_msg = { ...oldValues.in_stay_good_no_request_thanks_msg };
    valuesForm.request_activate = String(oldValues.in_stay_good_request_activate);
